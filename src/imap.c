/* $Id: imap.c,v 1.48 2003/12/07 15:36:38 ossi Exp $
 *
 ...
 */
imap_t *
imap_open (config_t * box, unsigned int minuid, imap_t * imap, int imap_create, int get_size)
{
  if (imap)
  {
 ...
 gotimap:
  ...
  else
  {
    ...
    if (imap->count > 0)
    {
      if (imap_exec (imap, "UID FETCH %d:* (FLAGS%s)", minuid, get_size?" RFC822.SIZE":""))
	goto bail;
    }
